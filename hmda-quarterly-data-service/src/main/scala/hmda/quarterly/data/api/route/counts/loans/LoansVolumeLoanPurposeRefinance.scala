...
import hmda.quarterly.data.api.route.counts.loans.Loans.{CATEGORY, LOAN_LABEL, LOAN_VOLUME_SUBTITLE, LOAN_VOLUME_TITLE_REFINANCE}
...
object LoansVolumeLoanPurposeRefinance extends GraphRoute(
  LOAN_VOLUME_TITLE_REFINANCE,
  CATEGORY,
  "loans-loan-purpose-refinance"
) with JsonSupport {
  ...
  private def getVolume(loanType: LoanTypeEnum, title: String, heloc: Boolean = false, conforming: Boolean = false): CancelableFuture[GraphSeriesSummary] =
    QuarterlyGraphRepo.fetchLoansVolumeByTypeLoanPurposeRefinance(loanType, heloc, conforming)
