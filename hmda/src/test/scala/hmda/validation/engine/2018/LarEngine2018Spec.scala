...
class LarEngine2018Spec extends WordSpec with ScalaCheckPropertyChecks with MustMatchers with Eventually {
  ...
  "Lar Validation Engine must pass all checks for a valid LoanApplicationRegister" in {
    eventually {
      ...
      val fixedLar =
        lar.copy(
          ...
          loan = lar.loan.copy(
            ...
            combinedLoanToValueRatio = "Exempt",
            debtToIncomeRatio = "NA",
            amount = 100000
          ),
          loanDisclosure = lar.loanDisclosure.copy(totalPointsAndFees = "NA", discountPoints = "NA", totalLoanCosts = "NA"),
          action = lar.action.copy(actionTakenType = PurchasedLoan, preapproval = PreapprovalNotRequested, actionTakenDate = 20180911),
      ...
      val testContext = ValidationContext(None, Some(Period(2018, None)))
